...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		//---------- 리스트 박스에 원하는 문자열 저장 --------------
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)TEXT("첫번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)TEXT("두번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)TEXT("세번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)TEXT("네번째문자열"));
		return 0;
	}
	...
	case WM_COMMAND:
	{
		if(LOWORD(wParam) == IDC_LISTBOX1)  //누가 통지했는가?
		{
